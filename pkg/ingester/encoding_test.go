	...
	for _, close := range []bool{true, false} {
		for _, tc := range []struct {
			...
			t.Run(fmt.Sprintf("%v-%s", close, tc.desc), func(t *testing.T) {
				conf := tc.conf
				c := chunkenc.NewMemChunk(chunkenc.ChunkFormatV4, compression.GZIP, chunkenc.UnorderedWithStructuredMetadataHeadBlockFmt, conf.BlockSize, conf.ChunkTargetSize, conf.ChunkMaxSize, conf.ChunkMinTime)
				fillChunk(t, c)
				if close {
